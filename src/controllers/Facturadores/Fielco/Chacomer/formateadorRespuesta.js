...
async function formateadorRespuesta(respuesta, body, transaccion_id) {
  try {
    ...
    const cabecera = {
      transaccion_id,
      nombre: respuesta.items.cliente,
      referencia,
    ...
    respuesta.cuotas.forEach((item) => {
      const { nroCuota, datoAdicional } = item;
      const descripcion = `Nro. Factura: ${datoAdicional.nro_factura}, Cuota Nro.: ${datoAdicional.cuota_nro}`;
      const fecha_vencimiento = datoAdicional.fecha_de_vencimiento;
      const monto = Number(datoAdicional.deuta_total);
