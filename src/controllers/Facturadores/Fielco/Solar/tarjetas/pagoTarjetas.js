...
const { formateadorRespuesta } = require('./formateadorRespuestaTarjetas');
...
const buscar = async (params) => {
  try {
    ...
    //   },
    //   data: { items: { fi3_1: params.body.referencia } },
    //   timeout: params.facturador.time_out,
    // });
    const result = require('./fakeResponse');
...
async function validarPago(params, headers, data) {
  const { fecha_vencimiento } = params.body;
  try {
    ...
    const { fechaVencimiento } = resultValidar.data;
