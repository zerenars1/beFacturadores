...
async function formateadorRespuesta(respuesta, body, transaccion_id) {
  try {
    ...
    respuesta.cuotas.forEach((item) => {
      ...
      const importeImpuesto = Number(datoAdicional.importe_impuesto);
      ...
      detalle.push({
        ...
        importe_mora_acumulado: (importeMora + importeImpuesto),
        importe_total_acumulado: (monto + importeImpuesto + importeMora),
