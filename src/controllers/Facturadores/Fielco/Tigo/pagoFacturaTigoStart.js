...
const { formateadorRespuesta } = require('./formateadorRespuestaTigoStart');
...
const consulta = async (params) => {
  try {
    ...
    return {
      success: false,
      data: {},
      message: error.message || 'No se pudo realizar la consulta solicitada',
    };
  }
};
...
async function validarPago(params, headers, data) {
  ...
  let montoPago = importe_total_acumulado;
  if (monto_recibido < importe_total_acumulado) {
    montoPago = monto_recibido;
  }
  try {
    ...
    const { monto } = resultValidar.data;
    if (Number(montoPago) !== Number(monto)) throw new Error('Error  al validar el importe');
  } catch (error) {
    if (error.isAxiosError && typeof error.response !== 'undefined') {
...
async function pago(params) {
  try {
    ...
    const ticket = await buildTicketBody({
      ...
      referenciaString: 'CI/NRO ABONADO',
