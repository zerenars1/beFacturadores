...
async function validarPago(params, headers, data) {
  ...
  try {
    ...
    if (monto !== resultValidar.data.monto) throw new Error('El importe no coincide con el del facturador');
  } catch (error) {
    if (error.isAxiosError && typeof error.response !== 'undefined') {
...
async function pago(params) {
  try {
    ...
    return {
      success: false,
      message: error.message || 'La operacion no pudo realizarse',
      data: {},
    };
  }
}
