...
async function formateadorRespuesta(respuesta, body, transaccion_id) {
  if (respuesta.error) {
    if (respuesta.error.code === 'PS-D-00') {
      return {
        success: false,
  ...
  if (respuesta.cuotas.length === 0) {
    return {
      success: false,
      message: 'El abonado no cuenta con cuotas pendientes de pago',
    };
  }
  ...
  try {
    ...
    respuesta.cuotas.forEach((item) => {
      // obtener el valor de tabla:${nrocuota}:key y convertir a json
      // para obtener la fecha de vencimiento, cada cuota
      let text;
    ...
    detalle.push({
      operacion: '',
      descripcion: 'Pago de cuota Electromax',
      cuota: cuotas,
    });
