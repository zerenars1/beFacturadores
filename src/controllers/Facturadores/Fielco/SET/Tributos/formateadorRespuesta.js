...
async function formateadorRespuesta(respuesta, body, transaccion_id) {
  try {
    ...
    const cabecera = {
      transaccion_id,
      nombre: '',
      referencia: respuesta.items.fi0_1,
    ...
    respuesta.cuotas.forEach((item) => {
      ...
      cabecera.nombre = datoRequerido[`tabla:${nroCuota}:descripcion`];
      const monto = Number(datoRequerido[`tabla:${nroCuota}:monto`]);
      const descripcion = `${datoAdicional.descripcion}, COD. IMPUESTO ${datoAdicional.cod_impuesto}, RESOLUCION ${datoAdicional.resolucion}`;

      detalle.push({
        numero_cuota: nroCuota,
        descripcion,
        fecha_vencimiento: datoAdicional.vencimiento,
